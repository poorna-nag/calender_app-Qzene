...
class _AgendaViewState extends State<AgendaView> {
  ...
  bool _shouldShowBadge(EventModel event, DateTime now) {
  ...
  Widget _buildStatusBadge(EventModel event, DateTime now, double fs) {
  ...
  String _getDurationString(EventModel event) {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<SettingsBloc, SettingsState>(
      builder: (context, settingsState) {
        final double fs = settingsState.settings.fontSize;
        return BlocBuilder<CalendarBloc, CalendarState>(
          builder: (context, calendarState) {
            final eventsMapRaw = calendarState is CalendarLoaded
            ...
            final eventsMap = <DateTime, List<EventModel>>{};
            final filterCounts = <AgendaFilter, int>{};
            for (var filter in AgendaFilter.values) filterCounts[filter] = 0;
              ...
              for (var filter in AgendaFilter.values) {
                ...
                switch (filter) {
                  ...
                  case AgendaFilter.next7Days:
                    inc =
                        !date.isBefore(today) && date.isBefore(next7DaysLimit);
                    break;
                ...
                if (inc)
                  filterCounts[filter] =
                      (filterCounts[filter] ?? 0) + events.length;
              }
                  ...
                  children: [
                    ...
                    Expanded(
                      child: sortedDates.isEmpty
                          ? _buildEmptyState(context, isDark, theme, fs)
                          : ListView.builder(
  ...
  Widget _buildDateHeader(
    ...
    String headerText = isToday
        ? 'Today – ${DateFormat('MMM d, yyyy').format(date)}'
        : isTomorrow
        ? 'Tomorrow – ${DateFormat('MMM d, yyyy').format(date)}'
        : '${DateFormat('EEEE').format(date)} – ${DateFormat('MMM d, yyyy').format(date)}';
    return Padding(
